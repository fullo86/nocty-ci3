...
class Snap extends CI_Controller {
	...
	public function token()
	{
		...
		foreach ($service as $selectedService) {
			if (empty($selectedService) || $selectedService == '') {
			}
			else {
				$detail_service = $this->M_service->detail($selectedService);
				$item_id = $i + 1;
				$item_details[$i] = [
					'id' => 'm' . $item_id,
					'price' => $detail_service->result_array()[0]['jml_bayar'],
					'quantity' => 1,
					'name' => $detail_service->result_array()[0]['service']
				];
	...
	public function cekJam(){
		if ($this->input->method() == 'post') {
			...
			$totalBookinganJamSama = $this->M_booking->cekJam(date('Y-m-d', strtotime($tanggal)), $jam);
			echo $totalBookinganJamSama;
		}
	}
}
